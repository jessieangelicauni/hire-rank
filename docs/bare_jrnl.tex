...
\begin{document}
\title{Hallucination-Aware Self-Correction for LLM-Based Applicant Assessment}
...
\markboth{Hallucination-Aware Self-Correction for LLM-Based Applicant Assessment}%
{Angelica \MakeLowercase{\textit{et al.}}: Hallucination-Aware Self-Correction}
...
\begin{IEEEkeywords}
AI Hallucination, Human resources, Large Language Models, Plackett-Luce model, Self-correction, Tournament Ranking.
\end{IEEEkeywords}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{Overall Architecture of the Proposed Applicant Ranking Methodology.png}
...
Job profiles are loaded as plain text and resumes are parsed from PDF. An LLM call extracts a normalized, resume-grounded list of skills from each applicant and each job profile, under explicit constraints that forbid inferring a skill from general job duties or naming a skill that is not literally present in the text. Every extracted skill is then embedded (L2-normalized) into a single shared matrix across the whole applicant corpus, so that later shortlisting for any job profile is one matrix multiplication rather than a per-applicant LLM call. Prior work on information retrieval has shown that combining lexical TF-IDF features with contextual BERT representations can improve retrieval classification performance [34]; this supports the broader use of contextual semantic representations for matching text beyond exact-keyword overlap.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\columnwidth]{Hallucination-Aware Self-Correction Mechanism for Applicant Assessment.png}
...
Per job profile, the pipeline runs several independent ``stability repeats'' of an active listwise tournament, following the MC-KG subset-selection and Plackett-Luce aggregation design of Yuksel et al. [1]. Each iteration: (1) samples applicant subsets not yet shown this repeat; (2) scores each subset via Monte Carlo draws from the current Laplace-approximated posterior over applicant utilities, each draw used only to simulate that subset's possible ranking outcome --- the resulting reduction in posterior variance (a knowledge-gradient rule) is then computed in closed form from the corresponding Hessian update rather than by refitting the model per draw, keeping the acquisition step cheap --- and picks the highest-scoring subset; (3) shows that subset's strengths, weaknesses, and additional skills to the LLM and asks it to rank them strongest to weakest; (4) refits the Plackett-Luce model on every ranking collected so far: a maximum a posteriori (MAP) estimate of each applicant's utility is found by gradient-based numerical optimization (L-BFGS-B) under a zero-mean Gaussian prior that gently regularizes the estimate toward zero, and the remaining uncertainty is a Laplace approximation, a Gaussian whose covariance is the inverse Hessian of the negative log-posterior at that MAP estimate, i.e. how sharply the fit is peaked. This MAP-plus-Laplace scheme is what makes step (2)'s knowledge-gradient rule tractable without full posterior sampling (e.g. MCMC) or bootstrap resampling at every iteration. A repeat stops when its iteration count is exhausted or every distinct subset has been shown once.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{MC-KG-Guided Tournament Ranking and Plackett-Luce Aggregation.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{Within-Repeat Convergence Kendall Tau Across Job Profiles.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{Final Kendall Tau by Job Role.png}
\caption{Final Kendall-$\tau$ by job role, averaged across stability repeats.}
\label{fig_convergence_by_role}
\end{figure}

Final Kendall's Tau per role, averaged across stability repeats (Fig.~5), spans 0.930 (frontend-engineer, n=7) to 0.979 (data-engineer, n=77), with smaller shortlists scoring lower --- consistent with Kendall's Tau being a noisier estimator over fewer candidates rather than evidence that smaller pools rank less reliably.
...
Of the 130 pairs with an initial contradiction (Table III), 88 were fixed by the retry and 42 still contradicted afterward and were dropped; a separate offline post-hoc audit of the run's final weaknesses found zero residual contradictions.
...
This paper's limitations are its synthetic resumes over synthetic job profiles authored by the paper's own authors, not real hiring data; its claims are scoped to pipeline-level reliability --- self-correction, ranking robustness, and convergence --- not real-world hiring validity or fairness. The results also lack validation against independent human-expert rankings, using a protocol such as [1]'s, so how these internal reliability gains translate to agreement with human judgment remains unmeasured. In addition, Faithfulness scoring uses the same Qwen2.5-14B model as generation rather than an independent judge, so shared-model bias cannot be fully ruled out.

The future work of this paper comprises two directions. 1) \emph{Memory-augmented self-correction}: giving the first-attempt generation access to a memory of previously-seen contradiction incidents, to test whether this reduces the first-attempt contradiction rate below this paper's 130/1,268 ($\approx$10.25\%) baseline. 2) \emph{DSPy-based prompt optimization}: tuning the generation prompt against the existing Faithfulness and contradiction-rate objectives, with domain-constrained generation for HR-tech as the specific claim. Together, these mechanisms advance a proven architecture --- the LLM listwise tournament with Plackett-Luce aggregation --- toward one whose failure modes on a real, locally-hosted model are documented, mitigated, and measured.
...
\bibitem{ref1}
K.~A. Yuksel, A.~B. Anees, A.~Elneima, S.~Hewavitharana, M.~Al-Badrashiny, and H.~Sawaf, ``Agentic AI for Human Resources: LLM-Driven Candidate Assessment,'' in \emph{Proc.\ 19th Conf.\ Eur.\ Chapter Assoc.\ Comput.\ Linguistics (EACL)}, vol.~3: System Demonstrations, Rabat, Morocco, Mar.~2026, pp.~341--348, doi: 10.18653/v1/2026.eacl-demo.24.
...
\bibitem{ref6}
J.~Li, B.~Xu, Z.~Chen, C.~Xu, M.~Chen, S.~Liu, Y.~Zhou, and Z.~Wen, ``Enhancing Talent Search Ranking with Role-Aware Expert Mixtures and LLM-based Fine-Grained Job Descriptions,'' in \emph{Proc.\ 2025 Conf.\ Empirical Methods in Natural Language Processing (EMNLP): Industry Track}, Suzhou, China, Nov.~2025, pp.~185--194, doi: 10.18653/v1/2025.emnlp-industry.13.

\bibitem{ref7}
O.~Walid, M.~T. Younes, K.~Shaban, M.~Hassan, and A.~Hamdi, ``MSLEF: Multi-Segment LLM Ensemble Finetuning in Recruitment,'' in \emph{Proc.\ 2025 IEEE/ACS 22nd Int.\ Conf.\ Computer Systems and Applications (AICCSA)}, Doha, Qatar, 2025, pp.~1--5, doi: 10.1109/aiccsa66935.2025.11315289.

\bibitem{ref8}
A.~Gera, S.~Reddy, A.~Kothapalli, A.~Milan K, and M.~Venugopalan, ``Nexus: A Multi-Modal Framework for Semantic Job Matching and AI-Driven Resume Analysis,'' \emph{Procedia Computer Science}, vol.~283, pp.~922--935, 2026, doi: 10.1016/j.procs.2026.06.173.
...
\bibitem{ref20}
B.~Malin, T.~Kalganova, and N.~Boulgouris, ``A review of faithfulness metrics for hallucination assessment in large language models,'' \emph{IEEE Journal of Selected Topics in Signal Processing}, vol.~19, no.~7, pp.~1362--1375, 2025, doi: 10.1109/JSTSP.2025.3579203.

\bibitem{ref21}
M.-H.~Ajjam and H.~S. Al-Raweshidy, ``AI-driven semantic similarity-based job matching framework for recruitment systems,'' \emph{Information Sciences}, vol.~724, art.~122728, 2026, doi: 10.1016/j.ins.2025.122728.

\bibitem{ref22}
X.~Liu, ``Deep learning-based intelligent resume-position matching system: Semantic understanding and recommendation of BERT model in massive recruitment data,'' in \emph{Proc.\ 2025 Int.\ Symp.\ Machine Learning and Social Computing (MLSC)}, Hong Kong, China, Oct.~2025, doi: 10.1145/3778450.3778452.

\bibitem{ref23}
C.~S. Mala, G.~Gezici, and F.~Giannotti, ``Disentangling faithfulness hallucinations in retrieval-augmented generation: A systematic benchmark and analysis,'' \emph{Machine Learning}, vol.~115, art.~187, 2026, doi: 10.1007/s10994-026-07121-y.
...
\bibitem{ref25}
L.~Alexander III, Q.~C. Song, L.~Hickman, and H.~J. Shin, ``Sourcing algorithms: Rethinking fairness in hiring in the era of algorithmic recruitment,'' \emph{International Journal of Selection and Assessment}, vol.~33, art.~e12499, 2025, doi: 10.1111/ijsa.12499.
...
\bibitem{ref27}
R.~J. Bandara, K.~Biswas, S.~Akter, S.~Shafique, and M.~Rahman, ``Addressing algorithmic bias in AI-driven HRM systems: Implications for strategic HRM effectiveness,'' \emph{Human Resource Management Journal}, vol.~35, no.~4, pp.~1047--1063, 2025, doi: 10.1111/1748-8583.12609.

\bibitem{ref28}
M.~Soleimani, A.~Intezari, J.~Arrowsmith, D.~J. Pauleen, and N.~Taskin, ``Reducing AI bias in recruitment and selection: An integrative grounded approach,'' \emph{The International Journal of Human Resource Management}, vol.~36, no.~14, pp.~2480--2515, 2025, doi: 10.1080/09585192.2025.2480617.

\bibitem{ref29}
A.~Rukadikar, K.~Khandelwal, and U.~Warrier, ``Reimagining recruitment: Traditional methods meet AI interventions --- A 20-year assessment (2003--2023),'' \emph{Cogent Business \& Management}, vol.~12, no.~1, art.~2454319, 2025, doi: 10.1080/23311975.2025.2454319.

\bibitem{ref30}
L.~Aminou, A.~Daaif, M.~Soulami, A.~Chalfaouat, and M.~Youssfi, ``Are cutting-edge technologies transforming classical approaches in automated hiring systems? A systematic literature review,'' \emph{Systems and Soft Computing}, vol.~7, art.~200421, 2025, doi: 10.1016/j.sasc.2025.200421.

\bibitem{ref31}
H.~Fahmi, A.~A. Nababan, M.~Jannah, M.~Khairani, Y.~F.~A. Lubis, and S.~Fahri, ``Analysis of Certainty Factor Methods to Determine Skills in Generation Y,'' in \emph{Proc.\ 2020 3rd Int.\ Conf.\ Mechanical, Electronics, Computer, and Industrial Technology (MECnIT)}, 2020, pp.~222--225, doi: 10.1109/MECnIT48290.2020.9166646.

\bibitem{ref32}
S.~Fahri, H.~Fahmi, J.~R. Sagala, Y.~Perwira, Y.~F.~A. Lubis, Nurjamiyah, and M.~Khairani, ``Using Certainty Factor Method to Determine Work Commitment Generation Y,'' \emph{Jurnal Teknik Komputer}, vol.~7, no.~1, pp.~124--129, 2021, doi: 10.31294/jtk.v7i1.10240.

\bibitem{ref33}
A.~Simangunsong, R.~M. Simanjorang, and H.~Fahmi, ``Penerapan Metode Composite Performance Index dalam Seleksi Penerimaan Calon Laboran,'' \emph{Jurnal Sistem Informasi, Teknik Informatika dan Teknologi Pendidikan}, vol.~1, no.~2, pp.~41--48, 2022, doi: 10.55338/justikpen.v1i2.8.

\bibitem{ref34}
P.~Aprilio, M.~Felix, P.~S. Nugraha, and H.~Fahmi, ``Hybrid Feature Combination of TF-IDF and BERT for Enhanced Information Retrieval Accuracy,'' \emph{JISA (Jurnal Informatika dan Sains)}, vol.~8, no.~1, pp.~8--15, 2025, doi: 10.31326/jisa.v8i1.2179.
