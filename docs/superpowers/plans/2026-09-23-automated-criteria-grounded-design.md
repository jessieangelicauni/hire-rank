# Automated, Adversarially-Validated Criteria-Grounded Question Design Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build an automated optimizer that revises the per-requirement Score-question criteria (`_REQUIREMENT_FIT_CRITERIA`), guarded against confidence-only Goodharting by an independent-model (Claude) proxy-label accuracy check and a red-team critic that mines low-confidence boundary cases, then compare the optimized criteria against the current hand-written baseline on held-out data.

**Architecture:** A round-based proposer/evaluate/critic loop. A DSPy-backed proposer (Qwen2.5-14B via Ollama) revises a 5-level criteria list each round; `evaluate_criteria` runs it through the real Jev API over a sample; `compute_metric` credits Jev's confidence only when it agrees with an independent Claude proxy label (the anti-Goodhart guard); `select_hard_triples` (the red-team critic) mines the lowest-confidence validation pairs each round and feeds them back as hard cases for the next round. A final script compares the optimizer's best criteria against the hand-written baseline on a held-out split.

**Tech Stack:** Python 3.12, Pydantic, DSPy (`dspy`) over Ollama (`qwen2.5:14b-instruct-q4_K_M`), Anthropic SDK (`anthropic`) for the proxy labeler, existing `JevClient`/`requests`, `scipy.stats` for the final Wilcoxon comparison, `pytest` + `unittest.mock`.
...
- No network calls in unit tests — mock `requests.post` (existing `JevClient` pattern), the Anthropic client, and the DSPy `Predict` call site, exactly as `tests/scoring/test_jev_client.py` already mocks `requests.post`.
- Reuse existing helpers instead of duplicating logic: `_build_state`, `_REQUIREMENT_FIT_CRITERIA` from `assessment.py`; `load_json_cache`/`save_json_cache` from `json_cache.py`. Cross-module import of underscore-prefixed helpers from `assessment.py` is an established pattern in this codebase (`scripts/run_jev_evaluation_study.py` already does it) — follow it, don't rename things to avoid the underscore.
- New model identifiers (Claude proxy-label model, DSPy/Ollama model) are config values with env overrides, not hardcoded strings, since exact API model IDs can change — follow the existing `RunConfig` / `_ENV_OVERRIDES` pattern in `config.py`.

---

## Task 1: Dependencies and config for the new integrations
...
**Interfaces:**
- Produces: `RunConfig.anthropic_api_key: str` (default `""`), `RunConfig.proxy_label_model: str` (default `"claude-opus-5"`), env vars `CANDIDATE_RANKING_ANTHROPIC_API_KEY`, `CANDIDATE_RANKING_PROXY_LABEL_MODEL`. Later tasks read these two fields to construct the Anthropic client and the DSPy `LM`.
...
- [ ] **Step 5: Add the new dependencies**
...
Run: `python -c "import dspy; import anthropic; print(dspy.__version__, anthropic.__version__)"`
...
git commit -m "feat: add DSPy and Anthropic dependencies and config for criteria optimization"
```
...
Create `tests/evaluation/__init__.py` (empty file, matches the package-less pattern of `tests/scoring/` — check first: run `ls tests/scoring/__init__.py 2>&1`; if it does not exist, skip creating `__init__.py` and rely on pytest's rootdir discovery instead, matching the existing convention exactly).
...
**Interfaces:**
- Consumes: `RequirementTriple`, `validate_criteria_shape` (Task 2); `JevQuestion`, `JevClient` (`candidate_ranking.scoring.jev_client`); `_build_state`, `_REQUIREMENT_KEY_PREFIX`-style instructions text from `candidate_ranking.scoring.assessment`; `JobDescription`, `Candidate` (`candidate_ranking.models`).
- Produces: `build_requirement_question(requirement: str, criteria: list[str]) -> JevQuestion`; `CriteriaEvalRecord` (pydantic model: `triple: RequirementTriple`, `score: float`, `confidence: float`); `evaluate_criteria(criteria: list[str], triples: list[RequirementTriple], jds_by_id: dict[str, JobDescription], candidates_by_id: dict[str, Candidate], jev_client: JevClient) -> list[CriteriaEvalRecord]`. Later tasks (metric, red-team, loop) consume `CriteriaEvalRecord`.
...
## Task 4: Anti-Goodhart metric and red-team hard-case selection
...
```bash
git add src/candidate_ranking/evaluation/criteria_optimization.py tests/evaluation/test_criteria_optimization.py
git commit -m "feat: add anti-Goodhart metric and red-team hard-case selection"
```
...
## Task 6: DSPy-backed criteria proposer
...
**Interfaces:**
- Consumes: `validate_criteria_shape` (Task 2).
- Produces: `ProposeCriteria` (`dspy.Signature`); `propose_criteria(current_criteria: list[str], hard_case_summaries: list[str], lm: "dspy.LM") -> list[str]`. Later tasks (optimization loop, Task 8; CLI script, Task 9) call `propose_criteria`.
...
git commit -m "feat: add DSPy-backed criteria proposer"
```
...
- Produces: `OptimizationRound` (pydantic model: `round_index: int`, `criteria: list[str]`, `metric: float`); `run_optimization(seed_criteria: list[str], train_triples: list[RequirementTriple], validation_triples: list[RequirementTriple], jds_by_id: dict[str, JobDescription], candidates_by_id: dict[str, Candidate], jev_client: JevClient, proxy_labels: dict[str, int], propose_fn: Callable[[list[str], list[str]], list[str]], max_rounds: int, hard_case_count: int, patience: int) -> tuple[list[str], list[OptimizationRound]]`. Later tasks (CLI script, Task 9) call `run_optimization` with a real `propose_criteria` bound to a real `dspy.LM`.
...
```bash
git add src/candidate_ranking/evaluation/criteria_optimization.py tests/evaluation/test_criteria_optimization.py
git commit -m "feat: add round-based criteria optimization loop with red-team refresh"
```
...
**Interfaces:**
- Consumes: everything from Tasks 1-8 (`RunConfig`/`apply_env_overrides`, `load_assessments_by_jd`, `load_requirement_triples`, `run_optimization`, `propose_criteria`, `ProxyLabelClient`, `JevClient`); `load_corpus` (copy the pattern from `scripts/run_jev_evaluation_study.py`).
...
```python
"""Run the automated, adversarially-validated criteria-design optimizer for the
...
def main(
    ...
    def bound_propose_fn(current_criteria: list[str], hard_case_summaries: list[str]) -> list[str]:
        return propose_criteria(current_criteria, hard_case_summaries, lm)
...
```python
"""Compare the DSPy-optimized per-requirement criteria against the hand-written
...
def main(run_id: str) -> None:
    ...
    # load_corpus + ProxyLabelClient wiring (Task 9) -- omitted here for brevity in review,
    # implementer wires it the same way before calling evaluate_criteria below.
...
In `docs/paper2_jev.tex`, after the existing §V-B (Criteria-Grounded Question Design results, ends around line 159 per the current file), add a new subsection reporting the held-out comparison. Do not fill in numeric results yet — that requires an actual run of `scripts/optimize_criteria.py` followed by `scripts/evaluate_criteria_comparison.py` against the real Jev and Anthropic APIs, which is a research execution step, not a code-writing step. Leave a clearly-marked run instruction as a LaTeX comment (not prose in the paper body — this is the one acceptable place for a placeholder, since it is an instruction to the paper's author, not paper text):

```latex
\subsection{Automated, Adversarially-Validated Criteria-Grounded Question Design}
...
This section reports an automated alternative: a round-based proposer/critic
loop (Section~III-C) that revises the per-requirement criteria under a
confidence-plus-independent-proxy-label metric, guarded against rewarding
confident-but-wrong answers, with a red-team critic that feeds the lowest-
confidence validation pairs back into each round as hard cases.
```
